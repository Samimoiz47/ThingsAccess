...
<head>
    ...
    <title>Blog - ThingsAccess</title>
    <meta name="description" content="Latest insights and articles from ThingsAccess. Explore our thoughts on IoT, enterprise software, and digital transformation.">
    <meta property="og:title" content="Blog - ThingsAccess">
    <meta property="og:description" content="Latest insights and articles from ThingsAccess. Explore our thoughts on IoT, enterprise software, and digital transformation.">
...
<body class="bg-black text-white min-h-screen">
    ...
    <section class="pt-32 pb-12 px-6 relative z-10">
        <div class="max-w-4xl mx-auto text-center">
            ...
            <p class="text-xl text-gray-300 mb-8">Latest insights and articles from our engineering team</p>
        </div>
    </section>

    <!-- Filters -->
    <section class="pb-12 px-6 relative z-10">
        <div class="max-w-7xl mx-auto">
            <!-- Category Filters -->
            <div class="mb-8">
                <h3 class="text-lg font-semibold mb-4">Filter by Category</h3>
                <div class="flex flex-wrap gap-3">
                    <a href="{{ route('blog.index') }}" class="px-4 py-2 text-sm rounded-full border border-white/20 hover:border-white/40 transition-all {{ !request()->has('category') ? 'filter-active' : '' }}">
                        All Categories
                    </a>
                    @foreach($categories as $category)
                    <a href="{{ route('blog.index', ['category' => $category->slug]) }}" class="px-4 py-2 text-sm rounded-full border border-white/20 hover:border-white/40 transition-all {{ request()->get('category') == $category->slug ? 'filter-active' : '' }}">
            ...
            <div class="mb-8">
                ...
                <div class="flex flex-wrap gap-3">
                    <a href="{{ route('blog.index') }}" class="px-4 py-2 text-sm rounded-full border border-white/20 hover:border-white/40 transition-all {{ !request()->has('tag') ? 'filter-active' : '' }}">
                        All Tags
                    </a>
                    @foreach($tags as $tag)
                    <a href="{{ route('blog.index', ['tag' => $tag->slug]) }}" class="px-4 py-2 text-sm rounded-full border border-white/20 hover:border-white/40 transition-all {{ request()->get('tag') == $tag->slug ? 'filter-active' : '' }}">
    ...
    <section class="pb-20 px-6 relative z-10">
        <div class="max-w-7xl mx-auto">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                ...
                <article class="glass-card rounded-2xl overflow-hidden group cursor-pointer flex flex-col h-full">
                    ...
                    <div class="p-6 flex flex-col flex-grow">
                        <div class="text-xs text-gray-400 mb-2">{{ $post->published_at ? $post->published_at->format('M j, Y') : 'Date not set' }} • {{ $post->read_time ?? '5' }} min read</div>
                        ...
                        <div class="flex items-center justify-between mt-auto">
                            <div class="flex items-center gap-2">
                                <div class="w-8 h-8 rounded-full bg-gradient-to-br from-gray-500 to-gray-700 flex items-center justify-center text-white font-bold text-xs">
                                    {{ strtoupper(substr($post->author->name, 0, 1)) }}
                                </div>
                                <span class="text-xs text-gray-400">{{ $post->author->name }}</span>
                            </div>
                            <a href="{{ route('blog.show', $post->slug) }}" class="text-white text-sm font-semibold hover:underline">Read Article &rarr;</a>
                        </div>
                        @if($post->tags->count() > 0)
                        <div class="flex gap-2 mt-4">
                            @foreach($post->tags as $tag)
                            <span class="px-2 py-1 bg-white/5 text-xs rounded-full">{{ $tag->name }}</span>
                            @endforeach
                        </div>
                        @endif
                    </div>
            ...
            <div class="text-center py-20">
                ...
                <p class="text-gray-400 mb-8">Try adjusting your filters or check back later for new content.</p>
                <a href="{{ route('blog.index') }}" class="px-6 py-3 bg-white text-black font-bold rounded-lg hover:bg-gray-200 transition-colors">
                    View All Posts
                </a>
            </div>
            @endif
